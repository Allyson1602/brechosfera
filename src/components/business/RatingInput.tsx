...
export function RatingInput({ businessId, businessName }: RatingInputProps) {
  ...
  const handleSubmitRating = async () => {
    ...
    if (!canRate) {
      toast.error("Você já avaliou uma loja recentemente. Tente novamente mais tarde.");
      return;
    }

    const numericBusinessId = Number(businessId);
    if (!Number.isFinite(numericBusinessId)) {
      toast.error("Não foi possível identificar a loja para registrar a avaliação.");
      return;
    }
